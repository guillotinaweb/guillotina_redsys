...
- A Guillotina **utility** to orchestrate Redsys REST calls (`IRedsysUtility`).
...
- Python **3.9+** (the codebase is compatible with 3.9; use `typing.Optional[...]` rather than PEP 604 unions at runtime).
- Guillotina
...
Add the app to Guillotina and enable Redis.

---

## Configuration

`guillotina_redsys.init` registers the utility and endpoints. Configure via env vars:

```python
# in your Guillotina settings
...
export REDSYS_SECRET_KEY=...               # Test/prod key
export REDSYS_URL=https://sis-t.redsys.es:25443/sis/rest
export REDSYS_CONTAINER_URL=https://your.app/db/container  # public base (Guillotina container URL)
```
...
**Returns:** decoded `RedsysIniciaPeticionResponse` as JSON (may include `threeDSMethodURL`, and `payload_3DS` for the browser).

---

### 2) Optional: 3DS Method kick-off (front-end oriented)
`POST  @initThreeDS` (resource)  
Accepts `transaction_id` (`threeDSServerTransID`) and `three_method_url`. The browser **should** post a form with `threeDSMethodData` to the issuer’s URL; this endpoint is provided mainly for testing. The recommended flow is to do this from the **frontend**.

**Body:**
```json
{
  "transaction_id": "uuid",
  "three_method_url": "https://.../threeDsMethod.jsp"
}
```

**Returns:** `{ "threeDSCompInd": "Y|N" }` (based on a 10s wait; in production you should record the method callback in Redis via the callback below and not rely on this call).
...
> Body example: `{ "CRES": "<base64url>" }` → stored in Redis under `notification_CRES:{order}:{trans}` (TTL 30 min).  
...
- `init_threeds_method(...)` → testing helper; for production, let the **browser** post `threeDSMethodData`.
...
- `authenticate_cres(...)` → reads the stored CRes and finalizes with `threeDSInfo="ChallengeResponse"`.

The utility uses `RestAPI`, an aiohttp client with retry/backoff.
...
> **Note:** This package keeps the bare minimum in Redis to correlate callbacks and finish the flow. _Card/PAN/CVV are passed again by the caller when finalizing_. If you wish to hold card data between steps, do so **encrypted** and purge immediately after finalization.
...
- If you persist card data between steps, **encrypt** and set short TTLs (and purge on completion).  
- Handle callbacks quickly and idempotently (return `200` fast and rely on Redis).  
- Ensure unique `order_id` per transaction or you’ll get Redsys `SIS0051` (duplicate order).
